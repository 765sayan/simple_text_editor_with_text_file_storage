import {
  FileDbInterface,
  ShareFileDbInterface,
  UserDbInterface,
} from "../interfaces/dbInterface";
...
export class FileDbImplementation implements FileDbInterface {
  ...
  async deleteFile(filename: string, creator: string) {
    ...
    if (file && file.length !== 0 && file.length <= 1) {
      try {
        const sharedFile = await ShareFile.find({
          primaryOwner: creator,
          fileName: file[0].id,
        });
...
export class ShareFileDbImplementation implements ShareFileDbInterface {
  async getAll() {
    try {
      const sharedFiles = await ShareFile.find().populate("fileName", [
        "filename",
      ]);
      if (sharedFiles) {
        return sharedFiles;
  ...
  async deleteSecondaryOwnerOfFileForUserDelete(
    sharedFileId: string,
    secondaryOwner: any,
    fileName: any
  ) {
    if (sharedFileId && secondaryOwner) {
      try {
        let sharedFile = await ShareFile.findById(sharedFileId).populate(
          "secondaryOwners",
          ["id", "username"]
        );
  ...
  async deleteSecondaryOwnerOfFile(
    userId: string | JwtPayload,
    secondaryOwner: any,
    filename: string
  ) {
    if (userId && secondaryOwner) {
      try {
        ...
        if (file && file.length !== 0) {
          let sharedFile = await ShareFile.find({
            primaryOwner: userId,
            fileName: { $eq: file[0].id },
          }).populate("secondaryOwners", ["username"]);

          if (sharedFile && sharedFile.length !== 0) {
            let secondaryOwners = sharedFile[0].secondaryOwners;
            let secondaryOwnerArraySecond = secondaryOwners.filter(
              (element) => {
  ...
  async getSharedFilesWithUser(userId: string | JwtPayload) {
    if (userId) {
      try {
        let sharedFiles = await ShareFile.find({
          secondaryOwners: { $all: [userId] },
        })
    ...
    if (userId && filename && secondaryOwner) {
      try {
        ...
        if (sharedFile && sharedFile.length === 1) {
          ...
          const shareFileModelEntry = await ShareFile.find({
            primaryOwner: userId,
            fileName: filename,
          });
          ...
          const shareFileModelEntry = await ShareFile.create({
            primaryOwner: userId,
