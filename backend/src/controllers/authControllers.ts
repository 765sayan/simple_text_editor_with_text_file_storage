...
export const registerController = async (req: Request, res: Response) => {
  ...
  if (username && password) {
    ...
    if (user) {
      ...
      const user = await userCrudInstance.createUser(
        username,
        encryptedPassword
      );
      if (user) {
...
export const editUserInfoController = async (req: Request, res: Response) => {
  ...
  if (userId && userData.username && userData.password) {
    ...
    const user = await userCrudInstance.updateUserInfo(
      userData.username,
      encryptedPassword,
      userId
    );
    ...
    if (var1 === 2) {
      ...
      if (sharedFiles) {
        for (let i = 0; i < sharedFiles?.length; i++) {
          await sharedFileCrudInstance.deleteSecondaryOwnerOfFileForUserDelete(
            sharedFiles[i].id,
            userId,
            sharedFiles[i].fileName
          );
        }
      }
